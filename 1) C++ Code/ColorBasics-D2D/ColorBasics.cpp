...
int CColorBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
	...
	std::ostringstream rgb;	// Folder Creation
	rgb << "C:/Patrick/GaitAnalysisPK/GaitAnalysisPK/Video/RGB/Subject_" << SUBJECT << "_Seq_" << RUN << "_Type_" << TYPE << "_Gen_" << GENDER;;
	std::string RGB = rgb.str();
	const char * c = RGB.c_str();
	mkdir(c);
...
void CColorBasics::ProcessColor(INT64 nTime, RGBQUAD* pBuffer, int nWidth, int nHeight) 
{
    ...
    if (pBuffer && (nWidth == cColorWidth) && (nHeight == cColorHeight))
    {
        ...
        if (m_bSaveScreenshot)
        {
            ...
            SetStatusMessage(szStatusMessage, 100, true);
...
HRESULT CColorBasics::GetScreenshotFileName(_Out_writes_z_(nFilePathSize) LPWSTR lpszFilePath, UINT nFilePathSize)
{
	...
	rgb << "C:/Patrick/GaitAnalysisPK/GaitAnalysisPK/Video/RGB/Subject_" << SUBJECT << "_Seq_" << RUN << "_Type_" << TYPE << "_Gen_" << GENDER;;
	std::string RGB = rgb.str();
	std::wstring WideRgb = std::wstring(RGB.begin(), RGB.end());

	const WCHAR* pszKnownPath = WideRgb.c_str();
