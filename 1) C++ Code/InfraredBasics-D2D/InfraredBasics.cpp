...
int CInfraredBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
	...
	std::ostringstream ir;	// Folder Creation
	ir << "C:/Patrick/GaitAnalysisPK/GaitAnalysisPK/Video/IR/Subject_" << SUBJECT << "_Seq_" << RUN << "_Type_" << TYPE << "_Gen_" << GENDER;;
	std::string IR = ir.str();
	const char * r = IR.c_str();
	mkdir(r);
...
HRESULT CInfraredBasics::GetScreenshotFileName(_Out_writes_z_(nFilePathSize) LPWSTR lpszFilePath, UINT nFilePathSize)
{
	...
	ir << "C:/Patrick/GaitAnalysisPK/GaitAnalysisPK/Video/IR/Subject_" << SUBJECT << "_Seq_" << RUN << "_Type_" << TYPE << "_Gen_" << GENDER;;
	std::string IR = ir.str();
	std::wstring WideIR = std::wstring(IR.begin(), IR.end());

	const WCHAR* pszKnownPath = WideIR.c_str();
	...
	if (SUCCEEDED(hr))
	{
		...
		StringCchPrintfW(lpszFilePath, nFilePathSize, L"%s\\KinectScreenshot-IR-%s.bmp", pszKnownPath, szTimeString);
	}
