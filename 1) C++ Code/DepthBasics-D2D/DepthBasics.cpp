...
int CDepthBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
	...
	std::ostringstream Depth;	// Folder Creation
	Depth << "C:/Patrick/GaitAnalysisPK/GaitAnalysisPK/Video/Depth/Subject_" << SUBJECT << "_Seq_" << RUN << "_Type_" << TYPE << "_Gen_" << GENDER;;
	std::string DEPTH = Depth.str();
	const char * d = DEPTH.c_str();
	mkdir(d);
...
HRESULT CDepthBasics::GetScreenshotFileName(_Out_writes_z_(nFilePathSize) LPWSTR lpszFilePath, UINT nFilePathSize)
{
  ...
  //Get location of folder
	...
	Depth << "C:/Patrick/GaitAnalysisPK/GaitAnalysisPK/Video/Depth/Subject_" << SUBJECT << "_Seq_" << RUN << "_Type_" << TYPE << "_Gen_" << GENDER;
	std::string DEPTH = Depth.str();
	std::wstring WideRgb = std::wstring(DEPTH.begin(), DEPTH.end());
